...
const InteractionBox = ({
  ...
  if (editMode)
    return (
      <div className="interaction-box vi-text-third flex items-center rounded-lg pt-0.5 ml-11 mr-6 pb-1 justify-between">
        <button
          onClick={() => onAddImage}
          className="action-btn vi-button flex items-center gap-1 px-3"
        >
  ...
  return (
    <div
      className={`interaction-box flex rounded-lg pt-0.5 pb-1 ${
        isMargin ? "vi-text-third ml-11" : ""
      }`}
    >
      {isLiked ? (
        <button
          onClick={onUnlikePoem}
          className="action-btn flex items-center vi-button px-3 gap-1"
        >
      ...
      {isSaved ? (
        <button
          onClick={onUnsavePoem}
          className="action-btn flex vi-button px-3 gap-1"
        >
