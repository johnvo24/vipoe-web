...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
  ({ className, type, icon, size, ...props }, ref) => {
    return (
      <div
        className={cn(
      ...
      >
        ...
        <input
          type={type}
          className={cn(
            `peer w-full ${ icon ? 'pl-11': 'pl-3' } ${ size == 1 ? 'py-2' : 'py-3' } pr-3 text-sm rounded-md border-none outline-none bg-transparent placeholder:text-muted-foreground focus:ring-0`,
            className
          )}
          ref={ref}
          {...props}
